...
但是我们有[json.dumps()](https://docs.python.org/3/library/json.html#basic-usage)，可以将**dict序列化为str。**
上面的做法可以写成
```python
...
这样就方便多了。
**注意**：
当字典转化为json时，所有的key都会被转换成字符串类型。所以当dict被dumps()成json，再loads()回来得到的字典，和原来的字典可能是不一样的。